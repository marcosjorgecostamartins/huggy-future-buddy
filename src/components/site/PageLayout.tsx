  ...
  return (
    <section className="grain blueprint bg-graphite pb-24 pt-44 text-cream md:pb-32 md:pt-56">
      <div className="mx-auto max-w-[1400px] px-6 lg:px-12">
        ...
        <h1 className="mt-10 max-w-[18ch] text-5xl leading-[0.98] md:text-7xl lg:text-[5.5rem]">{t(title)}</h1>
        {subtitle ? (
          <p className="mt-8 max-w-3xl text-xl text-cream/80 md:text-2xl">{t(subtitle)}</p>
        ) : null}
        {intro ? (
          <p className="mt-8 max-w-2xl text-base leading-relaxed text-institutional-soft">{t(intro)}</p>
        ) : null}
      </div>
    </section>
  );
  ...
  return (
    <section className="grain bg-forest-deep py-28 text-cream md:py-36">
      <div className="mx-auto flex max-w-[1400px] flex-col gap-12 px-6 lg:flex-row lg:items-end lg:justify-between lg:px-12">
        <h2 className="max-w-[16ch] text-4xl leading-[1.02] md:text-6xl">{t(title)}</h2>
