            ...
            <Reveal key={post.slug} index={Math.min(i, 3)}>
              ...
              >
                <div className="flex flex-col gap-2 font-mono text-[11px] uppercase tabular tracking-[0.16em] text-muted-foreground">
                  ...
                  <span className="text-emerald-action">{post.category}</span>
                ...
                <div>
                  ...
                  <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-muted-foreground">{post.excerpt}</p>
                </div>
              </Link>
            </Reveal>
          ))}
          {posts.length === 0 && (
            <p className="py-16 text-muted-foreground">Nenhum conteúdo nesta categoria por enquanto.</p>
          )}
        </div>
      </Section>
